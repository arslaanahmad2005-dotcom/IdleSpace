...
function BookingPage() {
  ...
  const safeHours = Math.max(1, Math.min(24, Math.floor(hours) || 1));
  const safeGuests = Math.max(1, Math.min(space.capacity, Math.floor(guests) || 1));
  const subtotal = space.price * safeHours;
  const fee = Math.round(subtotal * 0.09);
  const discount = coupon.trim().toLowerCase() === "welcome" ? Math.round(subtotal * 0.1) : 0;
            ...
            <div className="rounded-3xl bg-card border border-border overflow-hidden shadow-elegant">
              ...
              <div className="p-5">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-muted-foreground">{formatCurrency(space.price)} × {safeHours} hr</span><span>{formatCurrency(subtotal)}</span></div>
